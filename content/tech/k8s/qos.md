...
从`docker swarm`迁移到`k8s`已经有一段时间，花了很大的时间和精力，期间有过三次磁盘挂载异常的灾难级故障，而后又有周期性的节点故障。由于日常紧急开发任务比较多，每次遇到故障都是直接移除节点再添加，或者在开发和测试环境直接是强制重启节点以恢复正常运行。起初猜测是没有进行服务质量等级调优导致个别节点被调度太多服务，导致节点宕机。后面一次深入查证果然是因为内存溢出导致的各种诡异问题的出现。现在有时间回头处理这个事情了。顺便记录一下。
